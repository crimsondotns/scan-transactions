...
- ปุ่ม Slip ท้ายแผงขวา (คู่กับ View on explorer) → แผงเดิมสลับเป็นโหมดพรีวิวสลิป (ไม่มีไดอะล็อก, `useSlipView`): หัวมีปุ่มย้อนกลับ, body = ภาพ + รหัส, foot = Copy link / Print / Copy as image / Download PNG; ตรวจสลิปจากไอคอนโล่บนหัว (`#/v/<code>.<data>` เปิดไดอะล็อกตรวจ)
