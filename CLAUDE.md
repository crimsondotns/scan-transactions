...
- สลิป (`src/slip.ts`): วาดด้วย Canvas 2D ขาว/ดำเสมอ (เอกสาร ไม่ตามธีม) + QR (`qrcode`) ของลิงก์ explorer หรือ hash; รหัสยืนยัน = SHA-256 ของฟิลด์เนื้อหา (ชื่อเชน/ป้ายกระเป๋า/ลิงก์ไม่อยู่ในแฮช); สำเนาใน `xcap.scan.slips` ไม่เขียนทับ; ลิงก์แชร์ `#/v/<code>.<base64url(json)>` พกข้อมูลไปเอง → ตรวจได้ทุกเครื่องที่เปิดแอปนี้ ไม่มีเซิร์ฟเวอร์ ไม่มี RPC
- ปุ่ม Slip อยู่ท้ายแผงขวา (คู่กับ View on explorer); ตรวจสลิปจากไอคอนโล่บนหัว; พิมพ์/PDF ผ่าน `window.print()` + `@media print` (ซ่อนทุกอย่างยกเว้น `.slip-print`)
