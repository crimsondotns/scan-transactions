...
import { detectEndpoint } from '../src/store.ts';
...
test('frontend: alias templates resolve to the wrapper, absolute URLs untouched', () => {
  setWrapper(WRAP);
...
test('settings accept an alias template as a source', () => {
  assert.deepEqual(detectEndpoint('main/v1/transfers/{address}'), { name: 'main', family: 'evm' });
  assert.equal(detectEndpoint('solmain/v1/sol/activities')?.family, 'sol');
